...
function buildJudgeInput(args: {
  blueprints: GeneratedBlueprint[];
  criteria: CriteriaItemV0[];
  scale: { min: number; max: number };
}) {
  const criteriaLines = (args.criteria || []).map((c) => `- ${c.id}: ${c.text}`).join('\n');
  return [
    'You are grading a generated "blueprint set" against a fixed rubric.',
    ...
    '',
    'Blueprints JSON:',
    JSON.stringify(args.blueprints, null, 2),
    '',
...
async function scoreWithOpenAI(args: {
  ...
  const response = await client.responses.create({
    ...
    input: buildJudgeInput({ blueprints: args.blueprints, criteria: args.criteria, scale: args.scale }),
  });
...
export const llmGoldenRegressionBlueprintsV0: EvalClass<GeneratedBlueprint[], Record<string, unknown>> = {
  id: 'llm_golden_regression_blueprints_v0',
  run: async (
    blueprintsIn: GeneratedBlueprint[],
    ...
    if (!apiKeyPresent) {
      ...
      return mkEvalResult(
        'llm_golden_regression_blueprints_v0',
        ok,
        ok ? 'warn' : 'hard_fail',
        0,
    ...
    if (!domainId) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'missing_domain_id', {
    ...
    if (!goldenFixtureId) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'missing_golden_fixture_id', {
        expected: 'params.golden_fixture_id',
      });
    }

    const evalIdConst = 'llm_golden_regression_blueprints_v0';
    ...
    try {
      goldenDir = resolveDomainAsset(domainId, 'golden/blueprints');
    } catch (e) {
      const err = e instanceof Error ? e : new Error(String(e));
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'invalid_domain_asset_path', {
        domain_id: domainId,
    ...
    if (!fs.existsSync(goldenAbs)) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'missing_golden_fixture', {
        domain_id: domainId,
    ...
    let goldenRaw = '';
    let goldenList: GeneratedBlueprint[] | null = null;
    try {
      goldenRaw = fs.readFileSync(goldenAbs, 'utf8');
      const parsed = JSON.parse(goldenRaw);
      goldenList = Array.isArray(parsed) ? (parsed as GeneratedBlueprint[]) : ([parsed] as GeneratedBlueprint[]);
    } catch {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'invalid_golden_fixture_json', {
        domain_id: domainId,
        expected_path: path.join(goldenDir.relPath, goldenFixtureId).replace(/\\/g, '/'),
      });
    }
    ...
    if (!fs.existsSync(globalPackPath)) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'missing_global_pack', {
        expected_path: globalPackPath,
      });
    }
    const globalPack = readJsonStrict<MethodGlobalPackV0>(globalPackPath);
    if (Number((globalPack as any)?.version || 0) !== 0) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'bad_global_pack_version', {
        path: globalPackPath,
      });
    }
    ...
    if (domainPack && Number((domainPack as any)?.version || 0) !== 0) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'bad_domain_pack_version', {
        path: domainPackPath,
      });
    }
    ...
    if (!criteria.length) {
      return mkEvalResult('llm_golden_regression_blueprints_v0', false, 'hard_fail', 0, 'no_criteria', {
    ...
    const candidateRes = await scoreWithOpenAI({
      ...
      criteria,
      scale,
      blueprints: Array.isArray(blueprintsIn) ? blueprintsIn : [],
    });
    ...
    return mkEvalResult('llm_golden_regression_blueprints_v0', ok, ok ? 'info' : 'warn', ok ? score : 0, reason, data);
  },
};
