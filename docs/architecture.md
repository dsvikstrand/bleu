...
- Frontend:
  ...
  - Subscription management surface in `src/pages/Subscriptions.tsx` (Step 2: add + active/inactive actions for sync/deactivate/reactivate).
- Backend:
...
2. Subscription create/reactivate:
   ...
   - user manages existing rows on `/subscriptions` (`Sync now`, `Deactivate`, `Reactivate`).
